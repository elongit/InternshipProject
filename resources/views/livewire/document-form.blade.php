<fieldset class="w-full md:w-2/3 m-auto my-10 rtl:text-right shadow-md rounded" dir="rtl">
    <legend class=" text-blue-600 text-xl font-semibold p-2">اضافة الملف</legend>
    <form class="flex flex-col p-5 gap-5" enctype="multipart/form-data" wire:submit.prevent="store">
        ...
        <div class="grid grid-cols-2 gap-5">
            <div class="flex flex-col gap-2">
                ...
                <x-form-input wire:model="document_title" type="text" name="document_title" id="document_title" required placeholder="اسم الملف" />
                <x-form-error name="document_title" />
            </div>

            <div class="flex flex-col gap-2">
                <x-form-label name="document_type">نوع الملف </x-form-label>
                <x-form-input wire:model="document_type" type="text" name="document_type" id="document_type" required placeholder="نوع الملف" />
        ...
        <div class="flex flex-col gap-2 my-5 cursor-pointer">
            ...
            <input wire:model="uploaded_document" type="file" name="uploaded_document" class='bg-slate-200 p-3' required id="uploaded_document" />
        ...
        <div class="grid grid-cols-2 gap-5">
            <div class="flex flex-col gap-2">
                ...
                <x-form-input wire:model="date_archived" type="date" name="date_archived" required id="date_archived" />
                <x-form-error name="date_archived" />
            </div>

            <div class="flex flex-col gap-2">
                <x-form-label name="document_code">رمز الملف </x-form-label>
                <x-form-input wire:model="document_code" type="number" name="document_code" required id="document_code" min="1" />
        ...
        <div class="flex flex-col gap-2">
            ...
            <select wire:model="division_id" name="division_id" id="division_id" class="bg-slate-200 p-3" required>
                <option value="">
                    اختر الشعبة التي سيتم ادراج ملف اليها
                </option>
        ...
        <div class="grid grid-cols-2 gap-5">
            ...
            <div class="flex flex-col gap-2">
                <x-form-label name="box_id">العلبة</x-form-label>
                <select wire:model="box_id" name="box_id" id="box_id" class="bg-slate-200 p-3" required>
                    <option value="">
                        اختر العلبة
                    </option>
                    </option>
